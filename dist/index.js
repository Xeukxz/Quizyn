$(function () {
    var doGeneral = false, doQuotes = true;
    ...
    function gamerSort(val) {
        ...
        while (sorted.length < val.length) {
            ...
            for (var i = 0; i < val.length; i++)
                if (!sorted.includes(val[i]) && val[i] < lowest)
                    lowest = val[i];
            sorted.push(lowest);
        }
        return sorted;
    ...
    function extractText(topic, count) {
        ...
        if (args.length - 1 > random)
            ...
            vals.push(random - 2, random - 1);
        if (vals.length < 3) {
            if (args.length - 1 > random + 1)
                vals.push(random + 2);
            else
                vals.push(random - 1);
        }
    ...
    function play() {
        if (doGeneral)
            ...
            extractText(quotes, Math.floor(Math.random() * 3) + 1);
    }
